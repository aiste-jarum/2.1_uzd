...
int main()
{
    ...
    int vargk = 0, kietk = stud;
    for(int i = 1; i < kietk; i++)
    {
        if(naujas[i-1].galutinis < 5.0)
        {
            varg.push_back (naujas[i-1]);
            vargk++;
        }
    }
    naujas.erase(
          std::remove_if(naujas.begin(),naujas.end(),[](studentai v){return v.galutinis<5.0;}), naujas.end()
                 );
    ...
    std::string studg = std::to_string(naujas.size());
    ...
    rez2 << std::endl;
    for (int i = 0; i < naujas.size() -1; i++)
    {
        rez2 << std::setw(20) << std::left << naujas[i].vardas << std::setw(20) << std::left << naujas[i].pavarde
        << std::setw(10) << std::right << std::setprecision(3) << naujas[i].galutinis << std::setw(15) << std::right
        << std::setprecision(3) << naujas[i].mediana << std::endl;
    }
    rez2.close();
    std::cout << std::endl;
    if (ciklas < 4)
